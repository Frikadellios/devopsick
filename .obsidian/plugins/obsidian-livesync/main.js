...
var import_obsidian5 = require("obsidian"), ObsidianLiveSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    const b = s.createDiv(), E = b.createEl("h3", {
      ...
      text: ""
    }), S = "0.22.7", L = ~~(versionNumberString2Number(S) / 1e3), O = createSpan();
    ...
    new import_obsidian.Setting(k).setName("Minimal setup").addButton((r => {
      r.setButtonText("Start").onClick((async () => {
    ...
    const C = "vrtmrz/obsidian-livesync", T = "/docs/troubleshooting.md", I = `https://raw.githubusercontent.com/${C}/main`;
    k.createEl("div", "", (s => s.innerHTML = `<a href='https://github.com/${C}/blob/main${T}' target="_blank">Open in browser</a>`));
    const A = k.createEl("div", {
      text: "",
      cls: "sls-troubleshoot-preview"
    }), loadMarkdownPage = async (s, r = "") => {
      A.style.minHeight = A.clientHeight + "px";
      A.empty();
      const o = (s.startsWith("/") ? s : `${r}/${s}`).split("/"), u = o.pop(), g = o.join("/");
      let _ = "";
      try {
        _ = await (0, import_obsidian5.request)(`${I}${g}/${u}`);
      } catch (s) {
        _ = "Error Occurred!!\n" + s.toString();
      }
      const m = _.replace(/\((.*?(.png)|(.jpg))\)/g, `(${I}${g}/$1)`);
      await import_obsidian.MarkdownRenderer.render(this.plugin.app, `<a class='sls-troubleshoot-anchor'></a> [Tips and Troubleshooting](${T}) [PageTop](${u})\n\n${m}`, A, `${I}`, this.plugin);
      A.querySelector(".sls-troubleshoot-anchor").parentElement.setCssStyles({
        position: "sticky",
        top: "-1em",
        backgroundColor: "var(--modal-background)"
      });
      A.querySelectorAll("a.internal-link").forEach((s => {
        s.addEventListener("click", (async r => {
          const o = s.getAttr("data-href");
          if (o.startsWith("#")) {
            r.preventDefault();
            const s = Array.from(A.querySelectorAll("[data-heading]")).find((s => s.getAttr("data-heading").toLowerCase().split(" ").join("-") == o.substring(1).toLowerCase()));
            if (s) {
              s.setCssStyles({
                scrollMargin: "3em"
              });
    ...
    P.push(new import_obsidian.Setting(R).setName("URI").addText((s => s.setPlaceholder("https://........").setValue(this.plugin.settings.couchDB_URI).onChange((async s => {
      this.plugin.settings.couchDB_URI = s;
      await this.plugin.saveSettings();
    })))), new import_obsidian.Setting(R).setName("Username").setDesc("username").addText((s => s.setPlaceholder("").setValue(this.plugin.settings.couchDB_USER).onChange((async s => {
      this.plugin.settings.couchDB_USER = s;
      await this.plugin.saveSettings();
    })))), new import_obsidian.Setting(R).setName("Password").setDesc("password").addText((s => {
    ...
    })), new import_obsidian.Setting(R).setName("Database name").addText((s => s.setPlaceholder("").setValue(this.plugin.settings.couchDB_DBNAME).onChange((async s => {
      this.plugin.settings.couchDB_DBNAME = s;
      await this.plugin.saveSettings();
    })))));
    new import_obsidian.Setting(R).setName("Test Database Connection").setClass("wizardHidden").setDesc("Open database connection. If the remote database is not found and you have the privilege to create a database, the database will be created.").addButton((s => s.setButtonText("Test").setDisabled(false).onClick((async () => {
      await this.testConnection();
    }))));
    new import_obsidian.Setting(R).setName("Check and Fix database configuration").setDesc("Check the database configuration, and fix if there are any problems.").addButton((s => s.setButtonText("Check").setDisabled(false).onClick((async () => {
    ...
    const N = R.createEl("div", {
      text: ""
    });
    R.createEl("h4", {
      text: "Confidentiality"
    });
    const B = new import_obsidian.Setting(R).setName("End to End Encryption").setDesc("Encrypt contents on the remote database. If you use the plugin's synchronization feature, enabling this is recommend.").addToggle((s => s.setValue(r).onChange((async s => {
      if (D) {
    ...
    let F = this.plugin.settings.usePathObfuscation;
    const updateE2EControls = () => {
      B.controlEl.toggleClass("sls-item-dirty", r != this.plugin.settings.encrypt);
      if (r) {
        ...
        null == M || M.controlEl.toggleClass("sls-item-dirty", o != this.plugin.settings.passphrase);
        null == G || G.controlEl.toggleClass("sls-item-dirty", u != this.plugin.settings.useDynamicIterationCount);
        null == V || V.controlEl.toggleClass("sls-item-dirty", F != this.plugin.settings.usePathObfuscation);
      } else {
        M.settingEl.addClass("sls-setting-hidden");
        G.settingEl.addClass("sls-setting-hidden");
        V.settingEl.addClass("sls-setting-hidden");
      }
      if (r != this.plugin.settings.encrypt || o != this.plugin.settings.passphrase || u != this.plugin.settings.useDynamicIterationCount || F != this.plugin.settings.usePathObfuscation) j.settingEl.removeClass("sls-setting-hidden"); else j.settingEl.addClass("sls-setting-hidden");
    }, M = new import_obsidian.Setting(R).setName("Passphrase").setDesc("Encrypting passphrase. If you change the passphrase of a existing database, overwriting the remote database is strongly recommended.").addText((s => {
      ...
      s.inputEl.setAttribute("type", "password");
    })), V = new import_obsidian.Setting(R).setName("Path Obfuscation").setDesc("Obfuscate paths of files. If we configured, we should rebuild the database.").addToggle((s => s.setValue(F).onChange((async s => {
      if (D) {
    ...
    })))), G = new import_obsidian.Setting(R).setName("Use dynamic iteration count (experimental)").setDesc("Balancing the encryption/decryption load against the length of the passphrase if toggled.").addToggle((s => {
    ...
    })).setClass("wizardHidden"), j = new import_obsidian.Setting(R).setName("Apply").setDesc("Apply encryption settings").setClass("wizardHidden").addButton((s => s.setButtonText("Just apply").setWarning().setDisabled(false).onClick((async () => {
    ...
    const checkWorkingPassphrase = async () => {
      ...
      if (!r || "" != o) if (!r || await testCrypt()) {
        if (await checkWorkingPassphrase() || s) {
          ...
          this.plugin.settings.usePathObfuscation = F;
          this.plugin.settings.isConfigured = true;
          await this.plugin.saveSettings();
          updateE2EControls();
    ...
    new import_obsidian.Setting(R).setClass("wizardOnly").addButton((s => s.setButtonText("Next").setCta().setDisabled(false).onClick((() => {
      if (!this.plugin.settings.encrypt) this.plugin.settings.passphrase = "";
      if (isCloudantURI(this.plugin.settings.couchDB_URI)) this.plugin.settings.customChunkSize = 0; else this.plugin.settings.customChunkSize = 50;
      changeDisplay("30");
    }))));
    addScreenElement("0", R);
    const q = s.createDiv();
    q.createEl("h3", {
    ...
    let U, z = this.plugin.settings.settingSyncFile;
    new import_obsidian.Setting(q).setName("Filename").setDesc("If you set this, all settings are saved in a markdown file. You will also be notified when new settings were arrived. You can set different files by the platform.").addText((s => {
      s.setPlaceholder("livesync/setting.md").setValue(z).onChange((s => {
        z = s;
        if (z == this.plugin.settings.settingSyncFile) {
          U.removeCta();
          U.setDisabled(true);
        } else {
          U.setCta();
      ...
      s.setButtonText("Apply").onClick((async () => {
        this.plugin.settings.settingSyncFile = z;
        await this.plugin.saveSettings();
        this.display();
      }));
      U = s;
    ...
    new import_obsidian.Setting(q).setName("Encrypting sensitive configuration items").addDropdown((s => s.addOptions(W).setValue(this.plugin.settings.configPassphraseStore).onChange((async s => {
      this.plugin.settings.configPassphraseStore = s;
      this.plugin.usedPassphrase = "";
      K.setDisabled("LOCALSTORAGE" != this.plugin.settings.configPassphraseStore);
      await this.plugin.saveSettings();
    })))).setClass("wizardHidden");
    const H = localStorage.getItem("ls-setting-passphrase") || "", K = new import_obsidian.Setting(q).setName("Passphrase of sensitive configuration items").setDesc("This passphrase will not be copied to another device. It will be set to `Default` until you configure it again.").addText((s => {
      s.setPlaceholder("").setValue(H).onChange((async s => {
        this.plugin.usedPassphrase = "";
    ...
    new import_obsidian.Setting(Q).setName("Presets").setDesc("Apply preset configuration").addDropdown((s => s.addOptions({
      NONE: "",
      LIVESYNC: "LiveSync",
      PERIODIC: "Periodic w/ batch",
      DISABLE: "Disable all automatic"
    }).setValue(J).onChange((s => J = s)))).addButton((s => s.setButtonText("Apply").setDisabled(false).setCta().onClick((async () => {
      if ("" == J) {
    ...
    const Y = [], X = [];
    let Z = "";
    if (this.plugin.settings.liveSync) Z = "LIVESYNC"; else if (this.plugin.settings.periodicReplication) Z = "PERIODIC";
    new import_obsidian.Setting(Q).setName("Sync Mode").setClass("wizardHidden").addDropdown((s => s.addOptions({
      "": "On events",
      PERIODIC: "Periodic and On events",
      LIVESYNC: "LiveSync"
    }).setValue(Z).onChange((async s => {
      this.plugin.settings.liveSync = false;
      this.plugin.settings.periodicReplication = false;
    ...
    if (!this.plugin.settings.liveSync) {
      if (this.plugin.settings.periodicReplication) new import_obsidian.Setting(Q).setName("Periodic Sync interval").setDesc("Interval (sec)").setClass("wizardHidden").addText((s => {
      ...
      new import_obsidian.Setting(Q).setName("Sync on Save").setDesc("When you save file, sync automatically").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.syncOnSave).onChange((async s => {
        this.plugin.settings.syncOnSave = s;
        await this.plugin.saveSettings();
        applyDisplayEnabled();
      }))));
      new import_obsidian.Setting(Q).setName("Sync on Editor Save").setDesc("When you save file on the editor, sync automatically").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.syncOnEditorSave).onChange((async s => {
        this.plugin.settings.syncOnEditorSave = s;
        await this.plugin.saveSettings();
        applyDisplayEnabled();
      }))));
      new import_obsidian.Setting(Q).setName("Sync on File Open").setDesc("When you open file, sync automatically").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.syncOnFileOpen).onChange((async s => {
        this.plugin.settings.syncOnFileOpen = s;
        await this.plugin.saveSettings();
        applyDisplayEnabled();
      }))));
      new import_obsidian.Setting(Q).setName("Sync on Start").setDesc("Start synchronization after launching Obsidian.").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.syncOnStart).onChange((async s => {
        this.plugin.settings.syncOnStart = s;
        await this.plugin.saveSettings();
        applyDisplayEnabled();
      }))));
      new import_obsidian.Setting(Q).setName("Sync after merging file").setDesc("Sync automatically after merging files").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.syncAfterMerge).onChange((async s => {
    ...
    new import_obsidian.Setting(Q).setName("Use the trash bin").setDesc("Do not delete files that are deleted in remote, just move to trash.").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.trashInsteadDelete).onChange((async s => {
      this.plugin.settings.trashInsteadDelete = s;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian.Setting(Q).setName("Keep empty folder").setDesc("Normally, a folder is deleted when it becomes empty after a synchronization. Enabling this will prevent it from getting deleted").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.doNotDeleteFolder).onChange((async s => {
    ...
    new import_obsidian.Setting(Q).setName("Always overwrite with a newer file (beta)").setDesc("(Def off) Resolve conflicts by newer files automatically.").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.resolveConflictsByNewerFile).onChange((async s => {
      this.plugin.settings.resolveConflictsByNewerFile = s;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian.Setting(Q).setName("Postpone resolution of inactive files").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.checkConflictOnlyOnOpen).onChange((async s => {
      this.plugin.settings.checkConflictOnlyOnOpen = s;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian.Setting(Q).setName("Postpone manual resolution of inactive files").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.showMergeDialogOnlyOnActive).onChange((async s => {
    ...
    new import_obsidian.Setting(Q).setName("Always resolve conflict manually").setDesc("If this switch is turned on, a merge dialog will be displayed, even if the sensible-merge is possible automatically. (Turn on to previous behavior)").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.disableMarkdownAutoMerge).onChange((async s => {
      this.plugin.settings.disableMarkdownAutoMerge = s;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian.Setting(Q).setName("Always reflect synchronized changes even if the note has a conflict").setDesc("Turn on to previous behavior").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.writeDocumentsIfConflicted).onChange((async s => {
    ...
    new import_obsidian.Setting(Q).setName("Hidden file synchronization").setClass("wizardHidden").settingEl.createDiv("").innerText = this.plugin.settings.syncInternalFiles ? "🔁 : Enabled" : "⏹️ : Disabled";
    if (this.plugin.settings.syncInternalFiles) new import_obsidian.Setting(Q).setName("Disable Hidden files sync").setClass("wizardHidden").addButton((s => {
      s.setButtonText("Disable").onClick((async () => {
        this.plugin.settings.syncInternalFiles = false;
        await this.plugin.saveSettings();
        this.display();
      }));
    })); else new import_obsidian.Setting(Q).setName("Enable Hidden files sync").setClass("wizardHidden").addButton((s => {
    ...
    if (!this.plugin.settings.watchInternalFileChanges) new import_obsidian.Setting(Q).setName("Scan for hidden files before replication").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.syncInternalFilesBeforeReplication).onChange((async s => {
      this.plugin.settings.syncInternalFilesBeforeReplication = s;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian.Setting(Q).setName("Scan hidden files periodically").setDesc("Seconds, 0 to disable").setClass("wizardHidden").addText((s => {
    ...
    let ee = null;
    const te = "\\/node_modules\\/, \\/\\.git\\/, \\/obsidian-livesync\\/", ne = te + ",\\/workspace$ ,\\/workspace.json$,\\/workspace-mobile.json$";
    new import_obsidian.Setting(Q).setName("Folders and files to ignore").setDesc("Regular expression, If you use hidden file sync between desktop and mobile, adding `workspace$` is recommended.").setClass("wizardHidden").addTextArea((s => {
    ...
    new import_obsidian.Setting(Q).setName("Restore the skip pattern to default").setClass("wizardHidden").addButton((s => {
      s.setButtonText("Default").onClick((async () => {
        ee.setValue(te);
        this.plugin.settings.syncInternalFilesIgnorePatterns = te;
        await this.plugin.saveSettings();
      }));
    })).addButton((s => {
    ...
    new import_obsidian.Setting(Q).setName("Batch database update").setDesc("Reducing the frequency with which on-disk changes are reflected into the DB").setClass("wizardHidden").addToggle((s => s.setValue(this.plugin.settings.batchSave).onChange((async s => {
      this.plugin.settings.batchSave = s;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian.Setting(Q).setName("Enhance chunk size").setDesc("Enhance chunk size for binary files (Ratio). This cannot be increased when using IBM Cloudant.").setClass("wizardHidden").addText((s => {
    ...
    new import_obsidian.Setting(Q).setName("Fetch chunks on demand").setDesc("(ex. Read chunks online) If this option is enabled, LiveSync reads chunks online directly instead of replicating them locally. Increasing Custom chunk size is recommended.").setClass("wizardHidden").addToggle((s => {
    ...
    new import_obsidian.Setting(Q).setName("Synchronising files").setDesc("(RegExp) Empty to sync all files. set filter as a regular expression to limit synchronising files.").setClass("wizardHidden").addTextArea((s => {
    ...
    new import_obsidian.Setting(Q).setName("Non-Synchronising files").setDesc("(RegExp) If this is set, any changes to local and remote files that match this will be skipped.").setClass("wizardHidden").addTextArea((s => {
    ...
    new import_obsidian.Setting(Q).setName("Maximum file size").setDesc("(MB) If this is set, changes to local and remote files that are larger than this will be skipped. If the file becomes smaller again, a newer one will be used.").setClass("wizardHidden").addText((s => {
    ...
    new import_obsidian.Setting(Q).setName("(Beta) Use ignore files").setDesc("If this is set, changes to local files which are matched by the ignore files will be skipped. Remote changes are determined using local ignore files.").setClass("wizardHidden").addToggle((s => {
    ...
    if (this.plugin.settings.useIgnoreFiles) new import_obsidian.Setting(Q).setName("Ignore files").setDesc("We can use multiple ignore files, e.g.) `.gitignore, .dockerignore`").setClass("wizardHidden").addTextArea((s => {
    ...
    new import_obsidian.Setting(Q).setName("Batch size").setDesc("Number of change feed items to process at a time. Defaults to 50. Minimum is 2.").setClass("wizardHidden").addText((s => {
    ...
    new import_obsidian.Setting(Q).setName("Batch limit").setDesc("Number of batches to process at a time. Defaults to 40. Minimum is 2. This along with batch size controls how many docs are kept in memory at a time.").setClass("wizardHidden").addText((s => {
    ...
    new import_obsidian.Setting(Q).setName("Batch size of on-demand fetching").setDesc("").setClass("wizardHidden").addText((s => {
    ...
    new import_obsidian.Setting(Q).setName("The delay for consecutive on-demand fetches").setDesc("").setClass("wizardHidden").addText((s => {
    ...
    addScreenElement("30", Q);
    const ie = s.createDiv();
    ie.createEl("h3", {
      text: "Hatch"
    });
    new import_obsidian.Setting(ie).setName("Make report to inform the issue").addButton((s => s.setButtonText("Make report").setDisabled(false).onClick((async () => {
      let s = {};
      const r = "𝑅𝐸𝐷𝐴𝐶𝑇𝐸𝐷";
      try {
    ...
    new import_obsidian.Setting(ie).setName("Back to non-configured").addButton((s => s.setButtonText("Back").setDisabled(false).onClick((async () => {
    ...
    new import_obsidian.Setting(ie).setName("Verify and repair all files").setDesc("Compare the content of files between on local database and storage. If not matched, you will asked which one want to keep.").addButton((s => s.setButtonText("Verify all").setDisabled(false).setWarning().onClick((async () => {
      const s = this.app.vault.getFiles();
      let r = 0;
      for (const o of s) {
        r++;
        Logger(`${r}/${s.length}\n${o.path}`, LOG_LEVEL_NOTICE, "verify");
        if (!await this.plugin.isTargetFile(o)) continue;
        const u = await this.plugin.localDatabase.getDBEntry(o.path);
        if (!u) {
          Logger(`Compare: Not found on local database: ${o.path}`, LOG_LEVEL_NOTICE);
          continue;
        }
        let g;
        if ("newnote" == u.type) g = createBinaryBlob(await this.plugin.vaultAccess.vaultReadBinary(o)); else g = createTextBlob(await this.plugin.vaultAccess.vaultRead(o));
        if (isDocContentSame(g, u.data)) Logger(`Compare: SAME: ${o.path}`); else {
          Logger(`Compare: CONTENT IS NOT MATCHED! ${o.path}`, LOG_LEVEL_NOTICE);
          se.appendChild(se.createEl("div", {}, (s => {
            s.appendChild(s.createEl("h6", {
              text: o.path
            }));
            s.appendChild(s.createEl("div", {}, (s => {
              s.appendChild(s.createEl("div", {
                text: `Storage : Modified: ${new Date(o.stat.mtime).toLocaleString()}, Size:${o.stat.size}`
              }));
              s.appendChild(s.createEl("div", {
                text: `Database: Modified: ${new Date(u.mtime).toLocaleString()}, Size:${g.size}`
              }));
            })));
            s.appendChild(s.createEl("button", {
              text: "Storage -> Database"
            }, (r => {
              r.onClickEvent((() => {
                this.plugin.updateIntoDB(o, false, void 0, true);
                s.remove();
              }));
            })));
            s.appendChild(s.createEl("button", {
              ...
              r.onClickEvent((() => {
                this.plugin.pullFile(o.path, [], true, void 0, false);
                s.remove();
              }));
            })));
            return s;
    ...
    const se = ie.createDiv({
      text: ""
    });
    new import_obsidian.Setting(ie).setName("Check and convert non-path-obfuscated files").setDesc("").addButton((s => s.setButtonText("Perform").setDisabled(false).setWarning().onClick((async () => {
      var s, r, o;
    ...
    new import_obsidian.Setting(ie).setName("Delete all customization sync data").addButton((s => s.setButtonText("Delete").setDisabled(false).setWarning().onClick((async () => {
    ...
    new import_obsidian.Setting(ie).setName("Suspend file watching").setDesc("Stop watching for file change.").addToggle((s => s.setValue(this.plugin.settings.suspendFileWatching).onChange((async s => {
      this.plugin.settings.suspendFileWatching = s;
      await this.plugin.saveSettings();
      this.askReload();
    }))));
    new import_obsidian.Setting(ie).setName("Suspend database reflecting").setDesc("Stop reflecting database changes to storage files.").addToggle((s => s.setValue(this.plugin.settings.suspendParseReplicationResult).onChange((async s => {
      this.plugin.settings.suspendParseReplicationResult = s;
      await this.plugin.saveSettings();
      this.askReload();
    }))));
    new import_obsidian.Setting(ie).setName("Write logs into the file").setDesc("Warning! This will have a serious impact on performance. And the logs will not be synchronised under the default name. Please be careful with logs; they often contain your confidential information.").addToggle((s => s.setValue(this.plugin.settings.writeLogToTheFile).onChange((async s => {
      this.plugin.settings.writeLogToTheFile = s;
      await this.plugin.saveSettings();
    }))));
    new import_obsidian.Setting(ie).setName("Do not pace synchronization").setDesc("If this toggle enabled, synchronisation will not be paced by queued entries. If synchronisation has been deadlocked, please make this enabled once.").addToggle((s => s.setValue(this.plugin.settings.doNotPaceReplication).onChange((async s => {
    ...
    new import_obsidian.Setting(ie).setName("Do not keep metadata of deleted files.").setClass("wizardHidden").addToggle((s => {
    ...
    if (this.plugin.settings.deleteMetadataOfDeletedFiles) new import_obsidian.Setting(ie).setName("Delete old metadata of deleted files on start-up").setClass("wizardHidden").setDesc("(Days passed, 0 to disable automatic-deletion)").addText((s => {
    ...
    new import_obsidian.Setting(ie).setName("Use an old adapter for compatibility").setDesc("Before v0.17.16, we used an old adapter for the local database. Now the new adapter is preferred. However, it needs local database rebuilding. Please disable this toggle when you have enough time. If leave it enabled, also while fetching from the remote database, you will be asked to disable this.").setClass("wizardHidden").addToggle((s => s.setValue(!this.plugin.settings.useIndexedDBAdapter).onChange((async s => {
      this.plugin.settings.useIndexedDBAdapter = !s;
      await this.plugin.saveSettings();
      await rebuildDB("localOnly");
    }))));
    new import_obsidian.Setting(ie).setName("Scan changes on customization sync").setDesc("Do not use internal API").addToggle((s => s.setValue(!this.plugin.settings.watchInternalFileChanges).onChange((async s => {
      this.plugin.settings.watchInternalFileChanges = !s;
      await this.plugin.saveSettings();
    }))));
    let ae = this.plugin.settings.additionalSuffixOfDatabaseName + "";
    new import_obsidian.Setting(ie).setName("Database suffix").setDesc("LiveSync could not handle multiple vaults which have same name without different prefix, This should be automatically configured.").addText((s => {
      s.setPlaceholder("").setValue(ae).onChange((s => {
        ae = s;
      }));
    })).addButton((s => {
      s.setButtonText("Change").onClick((async () => {
        if (this.plugin.settings.additionalSuffixOfDatabaseName != ae) {
          this.plugin.settings.additionalSuffixOfDatabaseName = ae;
          await this.plugin.saveSettings();
    ...
    new import_obsidian.Setting(ie).setName("The Hash algorithm for chunk IDs").setDesc("xxhash64 is the current default.").setClass("wizardHidden").addDropdown((s => s.addOptions({
      "": "Old Algorithm",
    ...
    new import_obsidian.Setting(ie).setName("Fetch database with previous behaviour").setDesc("").addToggle((s => s.setValue(this.plugin.settings.doNotSuspendOnFetching).onChange((async s => {
      this.plugin.settings.doNotSuspendOnFetching = s;
      await this.plugin.saveSettings();
    }))));
    addScreenElement("50", ie);
    const re = s.createDiv();
    re.createEl("h3", {
      text: "Customization sync (beta)"
    });
    const oe = new import_obsidian.Setting(re).setName("Device name").setDesc("Unique name between all synchronized devices").addText((s => {
      ...
      oe.setDisabled(this.plugin.settings.usePluginSync);
    };
    updateDisabledOfDeviceAndVaultName();
    new import_obsidian.Setting(re).setName("Enable customization sync").addToggle((s => s.setValue(this.plugin.settings.usePluginSync).onChange((async r => {
    ...
    if (this.plugin.settings.usePluginSync) {
      new import_obsidian.Setting(re).setName("Scan customization automatically").setDesc("Scan customization before replicating.").addToggle((s => s.setValue(this.plugin.settings.autoSweepPlugins).onChange((async s => {
        this.plugin.settings.autoSweepPlugins = s;
        updateDisabledOfDeviceAndVaultName();
        await this.plugin.saveSettings();
      }))));
      if (!this.plugin.settings.watchInternalFileChanges) new import_obsidian.Setting(re).setName("Scan customization periodically").setDesc("Scan customization every 1 minute.").addToggle((s => s.setValue(this.plugin.settings.autoSweepPluginsPeriodic).onChange((async s => {
        this.plugin.settings.autoSweepPluginsPeriodic = s;
        updateDisabledOfDeviceAndVaultName();
        await this.plugin.saveSettings();
      }))));
      new import_obsidian.Setting(re).setName("Notify customized").setDesc("Notify when other device has newly customized.").addToggle((s => s.setValue(this.plugin.settings.notifyPluginOrSettingUpdated).onChange((async s => {
        this.plugin.settings.notifyPluginOrSettingUpdated = s;
        await this.plugin.saveSettings();
      }))));
      new import_obsidian.Setting(re).setName("Open").setDesc("Open the dialog").addButton((s => {
    ...
    updateDisabledOfDeviceAndVaultName();
    addScreenElement("60", re);
    const le = s.createDiv();
    le.createEl("h3", {
    ...
    new import_obsidian.Setting(le).setName("Lock remote database").setDesc("Lock remote database to prevent synchronization with other devices.").addButton((s => s.setButtonText("Lock").setDisabled(false).setWarning().onClick((async () => {
      await this.plugin.markRemoteLocked();
    }))));
    new import_obsidian.Setting(le).setName("Overwrite remote database").setDesc("Overwrite remote database with local DB and passphrase.").addButton((s => s.setButtonText("Send").setWarning().setDisabled(false).onClick((async () => {
      await rebuildDB("remoteOnly");
    }))));
    le.createEl("h4", {
      text: "The local database"
    });
    new import_obsidian.Setting(le).setName("Fetch rebuilt DB").setDesc("Restore or reconstruct local database from remote database.").addButton((s => s.setButtonText("Fetch").setWarning().setDisabled(false).onClick((async () => {
      await rebuildDB("localOnly");
    }))));
    new import_obsidian.Setting(le).setName("Discard local database to reset or uninstall Self-hosted LiveSync").addButton((s => s.setButtonText("Discard").setWarning().setDisabled(false).onClick((async () => {
      await this.plugin.resetLocalDatabase();
      await this.plugin.initializeDatabase();
    }))));
    le.createEl("h4", {
    ...
    new import_obsidian.Setting(le).setName("(Beta2) Clean up databases").setDesc("Delete unused chunks to shrink the database. This feature requires disabling 'Use an old adapter for compatibility'").addButton((s => s.setButtonText("DryRun").setDisabled(false).onClick((async () => {
    ...
    new import_obsidian.Setting(le).setName("Rebuild everything").setDesc("Rebuild local and remote database with local files.").addButton((s => s.setButtonText("Rebuild").setWarning().setDisabled(false).onClick((async () => {
      await rebuildDB("rebuildBothByThisDevice");
    }))));
    applyDisplayEnabled();
    addScreenElement("70", le);
    applyDisplayEnabled();
    if ("" == this.selectedScreen) if (L != this.plugin.settings.lastReadUpdates) if (this.plugin.settings.isConfigured) changeDisplay("100"); else changeDisplay("110"); else if (isAnySyncEnabled()) changeDisplay("20"); else changeDisplay("110"); else changeDisplay(this.selectedScreen);
  }
};
...
var LiveSyncLocalDB = class {
  ...
  async removeRevision(s, r) {
    try {
      ...
      if (isErrorOfMissingDoc(o)) {
        Logger(`Remove revision: Missing target revision, ${s}-${r}`, LOG_LEVEL_VERBOSE);
        return false;
      }
    }
  }
...
var databaseCache = {}, OpenKeyValueDatabase = async s => {
  ...
  constructor() {
    ...
    this.conflictResolutionProcessor = new QueueProcessor((async s => {
      ...
      try {
        ...
        await this.resolveByNewerEntry(s, u, g, m, b);
        return;
      } catch (s) {
  ...
  async watchVaultRawEventsAsync(s) {
    ...
    const o = normalizePath(this.app.vault.configDir);
    if ((!this.settings.usePluginSync ? [] : Object.values(this.settings.pluginSyncExtendedSetting).filter((s => s.mode == MODE_SELECTIVE || s.mode == MODE_PAUSED)).map((s => s.files)).flat().map((s => `${o}/${s}`.toLowerCase()))).some((r => r.startsWith(s.toLowerCase())))) {
      Logger(`Hidden file skipped: ${s} is synchronized in customization sync.`, LOG_LEVEL_VERBOSE);
      return;
    }
    const u = await this.vaultAccess.adapterStat(s);
    if (null != u && "file" != u.type) return;
    const g = null == u ? 0 : null != (r = null == u ? void 0 : u.mtime) ? r : 0, _ = ~~(g / 1e3), m = `${s}-${_}`;
    if (0 != g && this.recentProcessedInternalFiles.contains(m)) return;
    this.recentProcessedInternalFiles = [ m, ...this.recentProcessedInternalFiles ].slice(0, 100);
    const b = addPrefix(s, ICHeader), E = await this.localDatabase.getDBEntryMeta(b);
    if (~~((E && E.mtime || 0) / 1e3) != _) if (0 == _) await this.deleteInternalFileOnDatabase(s); else await this.storeInternalFileToDatabase({
      path: s,
      ...u
    });
  }
  ...
  async resolveByNewerEntry(s, r, o, u, g) {
    const _ = await this.localDatabase.getRaw(s, {
      rev: g
    }), m = ("mtime" in o && o.mtime || 0) < ("mtime" in _ && _.mtime || 0) ? u : g;
    await this.localDatabase.removeRevision(s, m);
    Logger(`Older one has been deleted:${r}`);
    if (0 == (await this.localDatabase.getRaw(s, {
      conflicts: true
    }))._conflicts.length) await this.extractInternalFileFromDatabase(stripAllPrefixes(r)); else this.conflictResolutionProcessor.enqueue(r);
  }
    ...
    if (!await this.plugin.isIgnoredByIgnoreFiles(s)) await serialized("file-" + u, (async () => {
      try {
        ...
        if (false === r) _ = {
          ...
          if ("_conflicts" in o) for (const u of o._conflicts) {
